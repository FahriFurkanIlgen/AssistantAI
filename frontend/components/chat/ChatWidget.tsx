...
export default function ChatWidget({
  ...
  const openPortfolio = async () => {
    if (portfolioPosts.length > 0) { setShowPortfolio(true); return; }
  ...
  const handlePortfolioImageClick = (post: PortfolioPost) => {
    ...
    if (post.url) {
      setInput((prev) => prev || (lang === "tr" ? "Bu stili beğendim, benzer bir şey istiyorum." : "I like this style, I want something similar."));
    }
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.type.startsWith("image/")) {
      toast.error(
        lang === "tr" ? "Lütfen bir görsel seçin." : "Please select an image.",
      );
      return;
    }
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="ml-auto flex items-center gap-3">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[11px] text-white/50">Çevrimiçi</span>
            </div>
          </div>
        </div>
      </div>
      ...
      {showPortfolio && (
        <div className="absolute inset-0 z-10 bg-white flex flex-col rounded-apple-lg overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-apple-border bg-apple-ink text-white">
            <div>
              <p className="font-display font-semibold text-[15px] tracking-tight">Portfolyo</p>
              {welcomeInfo?.instagram_handle && (
                <a
            ...
            <button onClick={() => setShowPortfolio(false)} className="text-white/60 hover:text-white text-xl leading-none">×</button>
          </div>
            ...
            {portfolioLoading ? (
              ...
              <>
                <p className="text-[12px] text-apple-secondary mb-2 px-1">
                  {lang === "tr" ? "Beğendiğinize tıklayın — asistana gönderin." : "Tap one you like to send it to the assistant."}
                </p>
